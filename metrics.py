...
class BaseMetric:
    ...
    def check_positive(number: float, name: str) -> float:
        if number < 0:
            raise ValueError(f"Error: Please povide a positive number for {name}.")
        else:
            return number

    def generate_p_values(
        self,
        true_alt: npt.NDArray[np.bool_],
        sample_size: int,
        ...
        p_values[true_alt] = self._generate_alt_p_values(
            total_alt, sample_size, random_state
        )
        p_values[~true_alt] = stats.uniform.rvs(
            0, 1, total_null, random_state=random_state
        )
...
class BooleanMetrics(BaseMetric):
    probability: float
    mde: float

    def __init__(self, probability: float, mde: float, alternative: str):
        super(BooleanMetrics, self).__init__(mde, alternative)
...
class NumericMetric(BaseMetric):
    ...
    def __init__(self, variance: float, mde: float, alternative: str):
    ...
    @property
    def power_analysis_instance(self) -> NormalIndPower | TTestIndPower:
        return TTestIndPower()
    
    
    def _generate_alt_p_values(self, size: int, sample_size: int, random_state: np.random.RandomState) -> npt.NDArray[np.float_]:
...
class RatioMetric(BaseMetric):
    ...
    def __init__(self, numerator_mean: float, numerator_variance: float, denominator_mean: float, denominator_variance: float, covariance: float, mde: float, alternative: str):
        super(RatioMetric, self).__init__(mde, alternative)
        self.numerator_mean = numerator_mean
        self.numerator_variance = self.check_positive(numerator_variance, "numerator_variance")
        self.denominator_mean = denominator_mean
        self.denominator_variance = self.check_positive(denominator_variance, "denominator_variance")
        self.covariance = covariance
        
    @property
    def variance(self) -> float:
        variance = (
            self.numerator_variance / self.denominator_variance**2 + self.denominator_variance * self.numerator_mean**2 / self.denominator_mean** 4 - 2 * self.covariance * self.numerator_mean / self.denominator_mean**3
        )
        return variance

...
from sample_size_calculator import DEFAULT_ALPHA
...
def get_mde(metric_type: str) -> float:
    ...
    mde = get_float(mde, "minimum detectable effect")
    return mde
...
def _get_metric() -> Dict[str, Collection[str]]:
    ...
    metric_metadata = get_metric_parameters(METRIC_PARAMETERS[metric])
    metric_metadata['mde'] = get_mde(metric)
    ...
    return {"metric_type": metric, "metric_metadata": metric_metadata}
